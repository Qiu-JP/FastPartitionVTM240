...
| `dataset/` | 网络可直接读取的训练、验证、测试和预训练数据集。 | 主要由 `network/src/createDataset.py` 根据 `video/` 和 `partition/` 生成；分类器预训练数据由对应的脚本生成。 | 主要供 `network/src/train.py` 使用。 |
...
其中亮度划分记录用于生成 64x64 亮度块对应的 gridmap 标签，以及 `Classifier_I` 所需的 CU 划分类型标签。

### `dataset/`

该目录保存网络训练阶段真正读取的数据。

对于 64x64 的亮度块主线，`network/src/createDataset.py` 会基于 `video/` 和 `partition/` 在真实数据集目录下生成 gridmap 预测网络所需的数据：

- 64x64 亮度 input；
- 对应的 2 通道、16x16 gridmap 标签。
...
`Luma_Input.pkl` 和 `Luma_Gridmap.pkl` 中各自保存一份样本 id。训练读取时会根据 `sequence_name, qp, frame_id, ctu_id` 作为唯一的id索引对齐输入数据和label。

`Classifier_I` 需要学习“局部 gridmap ROI -> 当前 CU 划分类型”的映射。它的数据集分为两类：

- 真实数据的 CU tree 节点标签：来自 VTM 实际划分记录，用于让分类器贴近真实编码分布；
- `pretrain/Classifier_I/logical/`：预训练数据，用人工构造的标准划分结构让分类器先学会基本划分模式，例如 NO_SPLIT、QT、BT、TT 的标准边界形态，由 `network/script/create_classifier_pretrain.py` 按规则生成。脚本会遍历 `Classifier_I` 支持的所有 gridmap ROI 尺寸，并为该尺寸下合法的划分类型构建标准gridmap样本和标签。
...
其中 `grid_y, grid_x, grid_h, grid_w` 表示该节点在 16x16 gridmap 上对应的 ROI 位置和尺寸，`label` 表示该 ROI 的标准划分类型。`Luma_CU_Tree.pkl` 中的 `samples` 负责把 `sequence_name, qp, frame_id, ctu_id` 映射到样本编号，并记录该样本在 `Luma_CU_Tree.npy` 中对应的节点范围。训练时先根据样本 id 对齐 `Luma_Input`、`Luma_Gridmap` 和 `Luma_CU_Tree`，再从预测或标签 gridmap 中裁剪节点 ROI 送入 `Classifier_I`。
...
其中 `DIV2K/training/` 与 `DIV2K/validating/` 是从真实视频和标准 VTM 划分结果转换出的训练/验证数据；`pretrain/Classifier_I/logical/<HxW>/` 是分类器逻辑预训练数据，每个尺寸目录下保存该 ROI 尺寸对应的标准 gridmap 样本和划分类型标签。旧版本数据中可能还会看到 `Luma_Ids.pkl` 或 `Luma_CU_Tree_Nodes.i2.bin`，它们不属于当前推荐的数据集结构，重新运行当前 `createDataset.py` 后应以 `*.pkl + *.npy` 为准。
