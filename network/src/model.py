...
class Classifier_I(nn.Module):
    """
    Intra luma CU split classifier for the encoder_intra_vtm.cfg constraints.
    ...
    Input shape is [B, 2, H, W], where H/W are gridmap units
    (CU pixels divided by 4). The 4x4 CU case has grid ROI 1x1 and
    does not need an MLP branch because no further split is legal.
    """
    def __init__(self, num_classes=6, input_channel=2, mask_value=-1e9):
        super().__init__()
        ...
        # Keys are gridmap ROI sizes: (grid_h, grid_w).
        ...
        for grid_size, hidden_channel in branch_specs.items():
            grid_h, grid_w = grid_size
            self.branches[self._size_key(grid_h, grid_w)] = nn.Sequential(
                nn.Linear(input_channel * grid_h * grid_w, hidden_channel),
                nn.ReLU(),
        ...
        for grid_size, mask in static_masks.items():
            grid_h, grid_w = grid_size
            self.register_buffer(
                "mask_" + self._size_key(grid_h, grid_w),
    ...
    def _static_mask(self, grid_h, grid_w):
        key = self._size_key(grid_h, grid_w)
        mask_name = "mask_" + key
        if not hasattr(self, mask_name):
            raise ValueError("Unsupported intra classifier grid ROI: {}x{}".format(grid_h, grid_w))
    ...
    def forward(self, x, dynamic_mask=None, return_probs=False):
        B, C, H, W = x.shape
        if C != self.input_channel:
            raise ValueError("Expected {} input channels, got {}".format(self.input_channel, C))

        static_mask = self._static_mask(H, W)
        key = self._size_key(H, W)

        if key in self.branches:
            logits = self.branches[key](x.reshape(B, -1))
        else:
            # 1x1 grid ROI corresponds to a 4x4 CU. It has no split branch.
